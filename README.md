...
- **`claude --resume` と相性よし(アンカー)** — 30秒以上走ったコマンド(claude等)が終了したペインは、そのプロジェクトフォルダに**錨を下ろして自動cdを停止**する。Finderでどれだけフォルダを見て回っても、戻ってくれば `--resume` がそのまま効く。自分で `cd` するか、メニューバーの「追従を再開」で解除(⚓マークで表示)
- **セッション保護** — 実行中プロセスがあるウィンドウを閉じると確認ダイアログ。「セッションを残す」で独立ウィンドウ化して継続できる
- **セッション一覧** — メニューバーアイコンに全ターミナルの一覧(フォルダ+実行中コマンド)。クリックでそのウィンドウ/タブを前面に呼び出せるので、claudeをどこで動かしていたか見失わない
- **⌥⌘T** で最前面のウィンドウ/タブのペインをトグル(全体のオン/オフはメニューバーから)
...
| ⌥⌘T | 最前面のウィンドウ/タブのペインをトグル |
| メニューバー > セッション一覧 | クリックでそのウィンドウ/タブを前面へ |
| メニューバー > 全ペインを表示 | 全体のオン/オフスイッチ |
